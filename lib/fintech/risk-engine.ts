...
export async function getLiquidityMonitorData(): Promise<LiquidityMonitorData> {
  ...
  const totalDeployedToday = todayInvoices.reduce((s, inv) => s + Number(inv.total), 0);
  const totalDeployedThisWeek = weekInvoices.reduce((s, inv) => s + Number(inv.total), 0);
  const totalDeployedThisMonth = monthInvoices.reduce((s, inv) => s + Number(inv.total), 0);
  ...
  const partnerBreakdown = partners.map((partner) => {
    ...
    const partnerTotal = partnerInvoices.reduce((s, inv) => s + Number(inv.total), 0);
  ...
  const platformRevenueYTD = ytdInvoices.reduce((s, inv) => s + Number(inv.total) * 0.015, 0);
  ...
  const velocityTotal = last7Days.reduce((s, inv) => s + Number(inv.total), 0);
