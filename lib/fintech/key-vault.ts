...
import { VaultSecretResponse } from "@/types/vault";
...
export async function resolveSigningSecret(
  ...
  if (vaultAddr && vaultToken) {
    try {
      ...
      const response = await fetch(
        `${vaultAddr}/v1/secret/data/${secretKey}${versionQuery}`,
        {
          method: "GET",
          headers: {
      ...
      const payload: VaultSecretResponse = await response.json();
      const secretValue = payload.data?.data?.value;

      if (!secretValue) {
        throw new Error(
          `Secret key "${secretKey}" was not found inside the Vault payload.`
        );
      }
  ...
  throw new Error(
    `Signing secret key "${secretKey}" could not be resolved by the Vault architecture.`
  );
}
