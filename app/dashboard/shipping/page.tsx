  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-1">
        <span className="text-[10px] text-foreground-muted">{progress}%</span>
        <span className="text-[10px] text-foreground-muted">{progress >= 100 ? "Complete" : "En Route"}</span>
      </div>
      ...
      <div className="flex items-center justify-between mt-1.5 px-0.5">
        <div className={`w-2 h-2 rounded-full ${progress >= 5 ? "bg-blue-400" : "bg-surface-raised"}`} title="Scheduled" />
        <div className={`w-2 h-2 rounded-full ${progress >= 50 ? "bg-accent-base" : "bg-surface-raised"}`} title="In Transit" />
        <div className={`w-2 h-2 rounded-full ${progress >= 100 ? "bg-emerald-400" : "bg-surface-raised"}`} title="Delivered" />
      </div>
    </div>
  );
}
...
function SkeletonCard() {
  return (
    <div className="rounded-xl border border-subtle bg-surface-raised p-4 animate-pulse">
      <div className="h-3 w-20 bg-surface-raised rounded mb-3" />
      <div className="h-6 w-24 bg-surface-raised rounded mb-2" />
    ...
    >
      ...
      <motion.div variants={fadeInUp} className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-foreground-tertiary mt-0.5">Fleet tracking, route optimization, and delivery management</p>
        </div>
      </motion.div>
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="w-8 h-8 rounded-lg bg-surface-raised flex items-center justify-center">
                  <m.icon size={15} className="text-foreground-tertiary" />
                </div>
              </div>
          ...
          {tripsLoading ? (
            <div className="animate-pulse space-y-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="h-32 bg-surface-raised rounded-xl border border-subtle" />
              ))}
            </div>
          ) : trips.length === 0 ? (
            ...
            <div className="space-y-3">
              {trips.map((trip) => {
                ...
                return (
                  <div key={trip.id} className="rounded-xl border border-subtle bg-surface-raised p-4 hover:bg-surface-raised transition-colors">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-[11px] font-mono text-foreground-tertiary">{trip.tripNumber}</span>
                          <StatusBadge status={trip.status} />
                        </div>
                        <p className="text-xs font-medium text-foreground">{destinations || "Direct Delivery"}</p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-foreground-tertiary">{new Date(trip.scheduledDate).toLocaleDateString()}</p>
                      </div>
                    </div>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-subtle bg-surface-raised p-4">
            <h3 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-2">
              <Truck size={14} className="text-foreground-tertiary" />
              Fleet Status
            </h3>
            {tripsLoading ? (
              <div className="animate-pulse space-y-3">
                {Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="h-14 bg-surface-raised rounded-lg" />
                ))}
              </div>
            ) : trips.length === 0 ? (
              ...
              <div className="space-y-3">
                {trips.slice(0, 6).map((trip) => (
                  <div key={trip.id} className="flex items-center justify-between p-3 rounded-lg border border-subtle hover:bg-surface-raised transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-xs font-medium text-foreground">{trip.vehiclePlate}</p>
                        <p className="text-[10px] text-foreground-muted">{trip.driverName}</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[10px] text-foreground-tertiary">{trip.stops.length} stops</p>
                      <p className="text-[9px] text-foreground-muted">{trip.status}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="rounded-xl border border-subtle bg-surface-raised p-4">
            ...
            <div className="aspect-video rounded-lg bg-surface-raised border border-subtle flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-[9px] text-foreground/15">Google Maps / Mapbox</p>
              </div>
            </div>
          </div>
        </div>
