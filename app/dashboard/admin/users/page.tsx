      ...
      <div className="border-b border-subtle">
        <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-[22px] font-bold tracking-tight text-foreground">User Management</h1>
              <p className="text-[13px] text-foreground-tertiary">View, search, and manage all platform users</p>
            </div>
          </div>
      ...
      <div className="px-6 py-6 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="h-10 px-3 rounded-lg text-sm text-foreground-tertiary bg-surface-raised border border-subtle outline-none focus:border-accent-base/40"
          >
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-subtle">
                  ...
                  <th className="px-5 py-3 text-[11px] font-semibold text-foreground-tertiary uppercase tracking-wider">Tenant</th>
                  <th className="px-5 py-3 text-[11px] font-semibold text-foreground-tertiary uppercase tracking-wider">Entity</th>
              ...
              <tbody className="divide-y divide-white/[0.04]">
                ...
                {users.map((user, i) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider border ${ROLE_COLORS[user.platformRole] || "bg-surface-raised text-foreground-tertiary border-subtle10"}`}>
                    ...
                    <td className="px-5 py-3.5">
                      <p className="text-[12px] text-foreground-tertiary">{user.tenant?.name || "—"}</p>
                    </td>
                    <td className="px-5 py-3.5">
                      {user.hotel && <p className="text-[12px] text-foreground-tertiary">{user.hotel.name}</p>}
                      {user.supplier && <p className="text-[12px] text-foreground-tertiary">{user.supplier.name}</p>}
                      {!user.hotel && !user.supplier && <span className="text-[12px] text-foreground-muted">—</span>}
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase ${STATUS_COLORS[user.status] || "bg-surface-raised text-foreground-tertiary"}`}>
                        {user.status}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
            ...
            <div className="px-5 py-3 border-t border-subtle flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={page <= 1}
                  className="p-1.5 rounded-lg text-foreground-tertiary hover:text-foreground hover:bg-surface-raised disabled:opacity-30 transition-colors"
                >
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <span className="text-[12px] text-foreground-muted px-2">
