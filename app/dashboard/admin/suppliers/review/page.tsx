  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Supplier Review</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        {[
          ...
          <div
            key={stat.label}
            className="p-4 rounded-xl border border-subtle bg-surface-raised"
          >
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-surface-raised border border-subtle text-foreground placeholder:text-foreground/15 focus:border-accent-base/40 focus:outline-none transition-colors text-sm"
          />
        </div>
      </div>
          ...
          <tbody>
            {loading ? (
              ...
              filtered.map((supplier) => (
                <tr key={supplier.id} className="border-b border-subtle hover:bg-surface-raised transition-colors">
                  <td className="px-4 py-3">
                    <div>
                      <p className="text-xs font-medium text-foreground">{supplier.name}</p>
                      <p className="text-[10px] text-foreground-muted">{supplier.email}</p>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs text-foreground-tertiary">{supplier.city}, {supplier.governorate}</span>
                  </td>
                  <td className="px-4 py-3">
                    <span className="text-xs font-mono text-foreground-tertiary">{supplier.taxId}</span>
                  </td>
                  <td className="px-4 py-3">
        ...
        {selectedSupplier && (
          <div className="space-y-5">
            ...
            <div className="p-4 rounded-xl bg-surface-raised border border-subtle">
              <h4 className="text-xs font-semibold text-foreground-tertiary uppercase tracking-wider mb-3 flex items-center gap-2">
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-surface-raised border border-subtle text-foreground-tertiary hover:text-red-400 hover:border-red-500/20 hover:bg-red-500/5 transition-colors text-sm font-medium"
              >
              ...
              <button
                ...
                disabled={actionLoading === selectedSupplier.id}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-accent-base hover:bg-accent-base/80 text-foreground text-sm font-medium transition-colors"
              >
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-foreground-tertiary">Provide a reason for rejection. The supplier will see this.</p>
          <textarea
            value={rejectReason}
            onChange={(e) => setRejectReason(e.target.value)}
            placeholder="Enter rejection reason..."
            rows={3}
            className="w-full px-3 py-2 rounded-lg bg-surface-raised border border-subtle text-sm text-foreground placeholder:text-foreground-muted focus:outline-none focus:border-accent-base/50 resize-none"
          />
          <div className="flex gap-3 justify-end">
            <button
              onClick={() => setRejectDialogSupplier(null)}
              className="px-4 py-2 rounded-lg bg-surface-raised border border-subtle text-sm text-foreground-tertiary hover:text-foreground transition-colors"
            >
              Cancel
            </button>
            <button
...
function DetailField({ label, value, colSpan = 1 }: { label: string; value: string; colSpan?: number }) {
  return (
    <div className={colSpan === 2 ? "col-span-2" : ""}>
      ...
      <p className="text-xs text-foreground/70 mt-0.5">{value}</p>
    </div>
  );
}
