...
export default function AdminDashboardPage() {
  ...
  const statusColor = (s: string) => {
    ...
    return map[s] || "bg-surface-raised text-foreground-muted border-subtle10";
  };
    ...
    <div className="min-h-screen">
      ...
      <div className="border-b border-subtle mb-8">
        <div className="py-6 flex items-center justify-between">
          <div>
            <h1 className="text-[24px] font-bold tracking-tight text-foreground">Command Center</h1>
            <p className="text-[13px] text-foreground-tertiary mt-1">Enterprise admin dashboard — all portals, all data, AI-powered</p>
          </div>
          <div className="flex items-center gap-3">
            <button className="relative p-2.5 rounded-xl bg-surface-raised border border-subtle text-foreground-muted hover:text-foreground hover:bg-surface-raised transition-all">
            ...
            <Link
              href="/admin/settings"
              className="p-2.5 rounded-xl bg-surface-raised border border-subtle text-foreground-muted hover:text-foreground hover:bg-surface-raised transition-all"
            >
      ...
      {/* Portal Switcher */}
      <div className="mb-8">
        <h2 className="text-[11px] font-semibold text-foreground-muted uppercase tracking-wider mb-3">Access Any Portal</h2>
          ...
          {PORTAL_SWITCHER.map((portal) => {
            ...
            return (
              <Link
                key={portal.label}
                href={portal.to}
                className="flex items-center gap-3 p-4 rounded-xl border border-subtle bg-[#0f0f0f] hover:border-subtle[0.12] hover:bg-surface-raised transition-all group"
              >
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground/80 group-hover:text-foreground transition-colors">{portal.label}</p>
                  <p className="text-[11px] text-foreground-muted">{portal.desc}</p>
                </div>
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="p-5 rounded-2xl bg-[#0f0f0f] border border-subtle animate-pulse">
              <div className="h-8 bg-surface-raised rounded w-16 mb-2" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {stats.map((stat) => (
            <motion.div
              ...
              className="p-5 rounded-2xl bg-[#0f0f0f] border border-subtle"
            >
              <div className="text-[28px] font-bold" style={{ color: stat.color }}>
                {stat.value.toLocaleString()}
              </div>
              <div className="text-[11px] text-foreground-tertiary mt-1">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        {/* Activity Feed */}
        <div className="lg:col-span-2 rounded-xl border border-subtle bg-[#0f0f0f]">
          <div className="p-5 border-b border-subtle flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-foreground/80">Activity Feed</h2>
            </div>
            <Link href="/admin/reports" className="text-[11px] text-foreground-muted hover:text-foreground-tertiary transition-colors">
              View All →
            </Link>
          </div>
            ...
            {actLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="p-4 animate-pulse">
                  ...
                  <div className="h-2 bg-surface-raised rounded w-2/3" />
                </div>
              ))
            ) : activity.length === 0 ? (
              ...
              activity.map((item) => (
                <div key={item.id} className="p-4 hover:bg-white/[0.01] transition-colors flex items-start gap-3">
                  <div className="p-1.5 rounded-md bg-surface-raised border border-subtle mt-0.5">
                    {typeIcon(item.type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <span className="text-sm font-medium text-foreground/70">{item.title}</span>
                      <span className="text-[10px] text-foreground-muted">{formatTimeAgo(item.timestamp)}</span>
                    </div>
                    <p className="text-xs text-foreground-muted">{item.description}</p>
                  </div>
                  {item.amount && (
                    <span className="text-xs font-medium text-foreground-tertiary whitespace-nowrap">
        ...
        <div className="space-y-6">
          {/* GMV Card */}
          {pulse && (
            <div className="p-5 rounded-xl bg-[#0f0f0f] border border-subtle">
              <p className="text-[11px] text-foreground-tertiary uppercase tracking-wider">Last 30 Days GMV</p>
              <p className="text-[28px] font-bold text-foreground mt-2">
              ...
              <Link
                href="/admin/reports"
                className="mt-4 block text-center px-4 py-2 bg-surface-raised border border-subtle text-foreground-tertiary text-[12px] font-medium rounded-lg hover:bg-surface-raised transition-colors"
              >
                View Full Reports
              </Link>
            </div>
          ...
          {/* Quick Links */}
          <div className="p-5 rounded-xl bg-[#0f0f0f] border border-subtle">
            <h3 className="text-sm font-semibold text-foreground-tertiary mb-3">Quick Actions</h3>
            <div className="space-y-2">
              {[
                ...
                return (
                  ...
                  >
                    <Icon className="w-4 h-4 text-foreground-muted group-hover:text-foreground-muted" />
                    <span className="text-sm text-foreground-muted group-hover:text-foreground/70 transition-colors">{link.label}</span>
      ...
      <div className="space-y-10">
        {MODULES.map((group, gi) => (
          <div key={group.group}>
            <h2 className="text-[14px] font-semibold text-foreground-muted uppercase tracking-wider mb-4">
              {group.group}
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {group.items.map((item, ii) => (
                ...
                >
                  <Link
                    href={item.to}
                    className="group block p-5 rounded-2xl bg-[#0f0f0f] border border-subtle hover:border-subtle[0.12] transition-all h-full"
                  >
                    <div
                    ...
                    <h3 className="text-[14px] font-semibold text-foreground mb-1.5 flex items-center gap-2">
                      {item.label}
                      <ArrowRight className="w-3.5 h-3.5 text-foreground-muted group-hover:text-foreground-muted group-hover:translate-x-0.5 transition-all" />
                    </h3>
