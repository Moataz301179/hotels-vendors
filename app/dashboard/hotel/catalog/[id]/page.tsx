...
export default function ProductDetailPage() {
  ...
  if (error || !product) {
    return (
      <div className="min-h-screen bg-black text-foreground flex flex-col items-center justify-center gap-4">
        <Package className="w-12 h-12 text-foreground-muted" />
        <h1 className="text-xl font-semibold">Product Not Found</h1>
        <p className="text-sm text-foreground-tertiary">{error || "This product may have been removed or the URL is incorrect."}</p>
        <Link href="/hotel/catalog" className="px-4 py-2 rounded-lg bg-accent-base hover:bg-accent-base/80 text-foreground text-sm font-medium transition-colors">
          Back to Catalog
        </Link>
      </div>
    );
      ...
      <div className="border-b border-subtle">
        <div className="max-w-[1400px] mx-auto px-6 py-3">
          <div className="flex items-center gap-2 text-sm text-foreground-tertiary">
            <Link href="/hotel" className="hover:text-foreground/70 transition-colors">Dashboard</Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <Link href="/hotel/catalog" className="hover:text-foreground/70 transition-colors">Catalog</Link>
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-foreground-tertiary">{product.name}</span>
          </div>
        </div>
      </div>

      <div className="max-w-[1400px] mx-auto px-6 py-8">
        <button onClick={() => router.push("/hotel/catalog")} className="flex items-center gap-2 text-sm text-foreground-tertiary hover:text-foreground/70 transition-colors mb-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
            <div className="relative aspect-square rounded-2xl border border-subtle bg-surface-raised overflow-hidden">
              {product.images && product.images.length > 0 ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-accent-base/5 to-transparent">
                  <Package className="w-20 h-20 text-foreground-muted mb-4" />
                  <span className="text-sm text-foreground-muted">{product.sku}</span>
                </div>
              )}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }} className="flex flex-col gap-5">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-foreground-muted font-mono">{product.sku}</span>
            </div>

            <h1 className="text-2xl font-bold tracking-tight">{product.name}</h1>
            {product.description && <p className="text-sm text-foreground-muted leading-relaxed">{product.description}</p>}
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-xs text-foreground-muted">({product.supplierReviewCount} reviews)</span>
              <span className="w-1 h-1 rounded-full bg-foreground-muted" />
              <span className="text-xs text-foreground-tertiary">{product.supplierCity}</span>
            </div>
            ...
            <div className="p-4 rounded-xl border border-subtle bg-surface-raised">
              <div className="flex items-baseline gap-3">
                ...
                <span className="text-sm text-foreground-tertiary">for {effectiveQty} {product.unitOfMeasure}</span>
              ...
              <p className="text-xs text-foreground-muted mt-1">{formatPrice(unitPrice)} / {product.unitOfMeasure} (bulk pricing applied)</p>
            </div>

            <div>
              <p className="text-xs font-semibold text-foreground-tertiary uppercase tracking-wider mb-2">Bulk Pricing</p>
                ...
                {bulkTiers.map((tier) => (
                  <button
                    ...
                    className={`p-2 rounded-lg border text-center transition-all ${
                      effectiveQty >= tier.qty ? "bg-accent-base/15 border-accent-base/30 text-foreground" : "bg-surface-raised border-subtle text-foreground-tertiary hover:text-foreground-tertiary"
                    }`}
                  >
                    <div className="text-xs font-bold">{tier.qty}+</div>
                    <div className="text-[10px] text-foreground-tertiary">{formatPrice(tier.price)}</div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center rounded-xl border border-subtle bg-surface-raised overflow-hidden">
                <button onClick={() => setQty(Math.max(product.minOrderQty, effectiveQty - 1))} className="px-4 py-3 text-foreground-tertiary hover:text-foreground hover:bg-accent-muted transition-colors"><Minus className="w-4 h-4" /></button>
                <span className="px-4 py-3 text-sm font-medium text-foreground min-w-[4rem] text-center">{effectiveQty}</span>
                <button onClick={() => setQty(Math.min(product.stockQuantity, effectiveQty + 1))} className="px-4 py-3 text-foreground-tertiary hover:text-foreground hover:bg-accent-muted transition-colors"><Plus className="w-4 h-4" /></button>
              </div>

              <button
                onClick={handleAddToCart}
                disabled={product.stockQuantity === 0}
                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-accent-base hover:bg-accent-base/80 disabled:bg-accent-muted disabled:text-foreground-muted text-foreground font-medium transition-all active:scale-[0.98]"
              >
              ...
              <button onClick={() => setIsWishlisted(!isWishlisted)} className={`p-3 rounded-xl border transition-colors ${isWishlisted ? "bg-accent-base border-accent-base text-foreground" : "border-subtle bg-surface-raised text-foreground-tertiary hover:text-foreground hover:border-accent"}`}>
              ...
              <button
                ...
                className={`p-3 rounded-xl border transition-colors ${inCompare ? "bg-accent-base border-accent-base text-foreground" : "border-subtle bg-surface-raised text-foreground-tertiary hover:text-foreground hover:border-accent"}`}
              >
            ...
            <p className="text-xs text-foreground-muted">
              Minimum order: <span className="text-foreground-muted">{product.minOrderQty} {product.unitOfMeasure}</span>
              {product.stockQuantity > 0 && <span> · Available: <span className="text-foreground-muted">{product.stockQuantity} {product.unitOfMeasure}</span></span>}
            </p>

            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
              <ShieldCheck className="w-4 h-4 text-emerald-400" />
              <span className="text-xs text-emerald-400">ETA E-Invoicing Compliant</span>
              <span className="text-[10px] text-foreground-muted">· Tax invoice auto-generated on order</span>
            </div>

            <div className="p-4 rounded-xl border border-subtle bg-surface-raised">
              <div className="flex items-center justify-between mb-3">
                <p className="text-xs font-semibold text-foreground-tertiary uppercase tracking-wider">Supplier</p>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase ${
                  ...
                  "bg-surface-raised text-foreground-tertiary border border-subtle"
          ...
          <div className="rounded-xl border border-subtle bg-surface-raised overflow-hidden">
            <div className="grid grid-cols-2 md:grid-cols-4">
              {[
                ...
                <div key={spec.label} className={`p-4 ${i < 4 ? "border-b" : ""} ${i % 4 !== 3 ? "border-r" : ""} border-subtle`}>
                  <p className="text-[10px] uppercase tracking-wider text-foreground-muted font-semibold mb-1">{spec.label}</p>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.title} className="flex items-start gap-3 p-4 rounded-xl border border-subtle bg-surface-raised">
