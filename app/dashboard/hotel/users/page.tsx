...
const ROLE_COLORS: Record<string, string> = {
  HOTEL_ADMIN: "bg-purple-500/10 text-purple-400 border-purple-500/20",
  HOTEL_MANAGER: "bg-blue-500/10 text-blue-400 border-blue-500/20",
  HOTEL_STAFF: "bg-surface-raised text-foreground-tertiary border-subtle10",
  HOTEL_VIEWER: "bg-surface-raised text-foreground-muted border-subtle10",
};
...
export default function HotelUsersPage() {
  ...
  const [inviteRole, setInviteRole] = useState("HOTEL_STAFF");
  ...
  const { data, loading, error, refetch } = useApi<UsersData>(`/api/v1/hotel/users?${queryParams.toString()}`);
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/v1/invite", {
        ...
        body: JSON.stringify({ email: inviteEmail, type: "HOTEL_USER", role: inviteRole }),
      });
  ...
  return (
    ...
    >
      <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="text-sm text-foreground-tertiary mt-0.5">Manage users who can access your hotel procurement portal</p>
          </div>
        </div>
        <button
      ...
      <motion.div variants={fadeInUp}>
        {loading ? (
          ...
          <EmptyState
            ...
            description="Invite users to collaborate on procurement."
            action={
              <button
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-br from-accent-base to-[#6B0000] flex items-center justify-center text-foreground text-[10px] font-bold flex-shrink-0">
                      ...
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wider border ${ROLE_COLORS[user.role] || "bg-surface-raised text-foreground-tertiary border-subtle10"}`}>
                          ...
                          {user.role.replace("HOTEL_", "")}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
                        <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase ${
                          user.status === "ACTIVE" ? "bg-emerald-500/10 text-emerald-400" : "bg-surface-raised text-foreground-tertiary"
                        }`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${user.status === "ACTIVE" ? "bg-emerald-400" : "bg-foreground-muted"}`} />
                          {user.status}
                        </span>
                      </td>
                      <td className="px-5 py-3.5">
            ...
            {pagination && pagination.totalPages > 1 && (
              <div className="px-5 py-3 border-t border-subtle flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1} className="p-1.5 rounded-lg text-foreground-tertiary hover:text-foreground hover:bg-surface-raised disabled:opacity-30 transition-colors">
                    <ChevronLeft className="w-4 h-4" />
                  </button>
                  <span className="text-[12px] text-foreground-muted px-2">{page} / {pagination.totalPages}</span>
                  <button onClick={() => setPage((p) => Math.min(pagination.totalPages, p + 1))} disabled={page >= pagination.totalPages} className="p-1.5 rounded-lg text-foreground-tertiary hover:text-foreground hover:bg-surface-raised disabled:opacity-30 transition-colors">
      ...
      {/* Invite Modal */}
      <Modal isOpen={inviteModalOpen} onClose={() => setInviteModalOpen(false)} title="Invite Team Member" description="Send an invite to join your hotel procurement team" size="sm">
        <form onSubmit={handleInvite} className="space-y-4">
          ...
          <div>
            ...
            >
              <option value="HOTEL_ADMIN" className="bg-[var(--background)]">Admin</option>
              <option value="HOTEL_MANAGER" className="bg-[var(--background)]">Manager</option>
              <option value="HOTEL_STAFF" className="bg-[var(--background)]">Staff</option>
              <option value="HOTEL_VIEWER" className="bg-[var(--background)]">Viewer</option>
            </select>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-accent-base hover:bg-accent-base/80 disabled:opacity-50 text-xs text-foreground font-medium transition-all"
          >
