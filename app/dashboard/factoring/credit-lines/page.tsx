...
export default function CreditLineApplicationPage() {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-[var(--background)] text-foreground flex items-center justify-center">
        <div className="text-center max-w-md px-6">
          <div className="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center mx-auto mb-6">
            <CheckCircle2 className="w-8 h-8 text-emerald-400" />
          </div>
          <h2 className="text-[24px] font-bold mb-3">Application Submitted</h2>
          <p className="text-foreground-muted text-[14px] leading-relaxed mb-2">
            Your credit line application is under review. Our AI auditor is analyzing your financial position.
          </p>
          <div className="bg-[#0f0f0f] border border-subtle rounded-xl p-4 mt-6 text-left">
            <div className="flex justify-between mb-2">
              <span className="text-[12px] text-foreground-tertiary">Credit Score</span>
              <span className="text-[14px] font-bold text-emerald-400">{calculateCreditScore()}/100</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[12px] text-foreground-tertiary">Recommended Limit</span>
          ...
          <button
            ...
            className="mt-8 px-6 py-2.5 bg-accent-base hover:bg-[#6B0000] text-foreground text-[13px] font-semibold rounded-lg transition-colors"
          >
            Back to Dashboard
          </button>
        </div>
      ...
      <div className="border-b border-subtle">
        <div className="mx-auto max-w-5xl px-6 py-6">
          <div className="flex items-center gap-3 mb-1">
            <button onClick={() => router.push("/factoring")} className="text-foreground-tertiary hover:text-foreground transition-colors">
              <ArrowLeft className="w-5 h-5" />
            </button>
            <h1 className="text-[22px] font-bold tracking-tight">Credit Line Application</h1>
          </div>
          <p className="text-[13px] text-foreground-tertiary ml-8">
      ...
      <div className="mx-auto max-w-5xl px-6 py-8">
                ...
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${
                    step === s.id
                      ...
                      : "bg-surface-raised border-subtle text-foreground-muted"
                  }`}
                >
                  {step > s.id ? <CheckCircle2 className="w-5 h-5" /> : <s.icon className="w-5 h-5" />}
                </div>
                <span className={`text-[11px] font-medium ${step >= s.id ? "text-foreground/70" : "text-foreground-muted"}`}>
                  {s.label}
                </span>
              </button>
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-[1px] mx-3 ${step > s.id ? "bg-emerald-500/30" : "bg-surface-raised"}`} />
              )}
            </div>
          ))}
        </div>
            ...
            <motion.div key="step1" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <SectionCard title="Hotel Profile" subtitle="Basic information and legal identity">
                ...
                <div className="mt-6 pt-6 border-t border-subtle">
                  <p className="text-[12px] font-semibold text-foreground-tertiary uppercase tracking-wider mb-4">Legal Documents</p>
                ...
                <div className="mt-6 pt-6 border-t border-subtle">
                  <p className="text-[12px] font-semibold text-foreground-tertiary uppercase tracking-wider mb-4">Key Contacts</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <SectionCard title="Financial Position" subtitle="Last 12 months. All figures in EGP.">
                ...
                {/* Score Preview */}
                <div className="mt-6 p-4 rounded-xl bg-[#0f0f0f] border border-subtle">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-[12px] text-foreground-tertiary">Preliminary Credit Score</p>
                      <p className="text-[28px] font-bold text-emerald-400">{calculateCreditScore()}/100</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[12px] text-foreground-tertiary">Recommended Limit</p>
                      <p className="text-[28px] font-bold text-foreground">EGP {calculateRecommendedLimit().toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </SectionCard>
                  ...
                  <div className="p-5 rounded-xl bg-emerald-500/5 border border-emerald-500/15">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[32px] font-bold text-foreground">EGP {calculateRecommendedLimit().toLocaleString()}</p>
                      </div>
                    </div>
        ...
        <div className="flex items-center justify-between mt-8">
          <button
            ...
            disabled={step === 1}
            className="px-5 py-2.5 border border-subtle text-foreground-tertiary text-[13px] font-medium rounded-lg hover:bg-surface-raised transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          >
            Previous
          </button>
          ...
          {step < 5 ? (
            <button
              ...
              className="px-6 py-2.5 bg-accent-base hover:bg-[#6B0000] text-foreground text-[13px] font-semibold rounded-lg transition-colors flex items-center gap-2"
            >
            ...
            <button
              ...
              disabled={submitting}
              className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-foreground text-[13px] font-semibold rounded-lg transition-colors flex items-center gap-2 disabled:opacity-50"
            >
...
function SectionCard({ title, subtitle, children }: { title: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="p-6 rounded-2xl bg-[#0f0f0f] border border-subtle">
      <h3 className="text-[16px] font-semibold text-foreground">{title}</h3>
      {subtitle && <p className="text-[12px] text-foreground-tertiary mt-1 mb-6">{subtitle}</p>}
      {children}
    </div>
  );
}
...
function Input({ label, type = "text", value, onChange }: { label: string; type?: string; value: string; onChange: (v: string) => void }) {
  return (
    <div>
      <label className="block text-[12px] font-medium text-foreground-muted mb-1.5">{label}</label>
      <input
        type={type}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="w-full px-3 py-2.5 bg-surface-raised border border-subtle rounded-lg text-[13px] text-foreground placeholder-white/20 focus:outline-none focus:border-subtle20 transition-colors"
      />
    </div>
  );
}
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          onChange={(e) => onChange(e.target.value)}
          placeholder="0"
          className="w-full pl-10 pr-3 py-2.5 bg-surface-raised border border-subtle rounded-lg text-[13px] text-foreground placeholder-white/20 focus:outline-none focus:border-subtle20 transition-colors"
        />
      </div>
...
function DocUpload({ label, file, onChange, required }: { label: string; file: File | null; onChange: (f: File | null) => void; required?: boolean }) {
  return (
    <div className="p-4 rounded-xl bg-surface-raised border border-subtle hover:border-subtle[0.1] transition-colors">
      <div className="flex items-center justify-between mb-3">
        <span className="text-[13px] text-foreground/70">{label} {required && <span className="text-accent-base">*</span>}</span>
        {file ? (
      ...
      <label className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg bg-surface-raised border border-dashed border-subtle[0.1] hover:border-subtle[0.2] cursor-pointer transition-colors">
        <Upload className="w-4 h-4 text-foreground-muted" />
        <span className="text-[12px] text-foreground-tertiary">{file ? "Replace" : "Upload"}</span>
...
function ToggleRow({ label, checked, onChange, desc }: { label: string; checked: boolean; onChange: (v: boolean) => void; desc?: string }) {
  return (
    <div className="flex items-start justify-between p-4 rounded-xl bg-surface-raised border border-subtle">
      <div>
        <p className="text-[13px] text-foreground/80 font-medium">{label}</p>
        {desc && <p className="text-[11px] text-foreground-muted mt-0.5">{desc}</p>}
      </div>
      <button
        onClick={() => onChange(!checked)}
        className={`relative w-11 h-6 rounded-full transition-colors ${checked ? "bg-emerald-500" : "bg-surface-raised"}`}
      >
...
function ReviewSection({ title, items }: { title: string; items: { label: string; value: string }[] }) {
  return (
    <div>
      <p className="text-[12px] font-semibold text-foreground-muted uppercase tracking-wider mb-3">{title}</p>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {items.map((item) => (
          <div key={item.label} className="p-3 rounded-lg bg-surface-raised border border-subtle">
            <p className="text-[10px] text-foreground-muted">{item.label}</p>
            <p className="text-[13px] text-foreground font-medium mt-0.5">{item.value}</p>
          </div>
        ))}
      </div>
    </div>
