    ...
    <div className="min-h-screen bg-[var(--background)] text-foreground">
        ...
        <div className="mx-auto max-w-7xl px-6 py-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[13px] text-foreground-tertiary mt-0.5">Review and approve hotel credit facilities</p>
            </div>
          </div>
            ...
            {["ALL", "PENDING_REVIEW", "AI_ANALYZING", "FACTORING_REVIEW", "APPROVED", "REJECTED"].map((s) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-[11px] font-medium transition-colors ${
                  filter === s ? "bg-surface-raised text-foreground" : "text-foreground-muted hover:text-foreground-tertiary"
                }`}
              >
      ...
      <div className="mx-auto max-w-7xl px-6 py-6">
        <div className="flex items-center gap-3 mb-6">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-surface-raised border border-subtle rounded-lg text-[13px] text-foreground placeholder-white/20 focus:outline-none focus:border-subtle20"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 text-foreground-muted text-[14px]">No applications found.</div>
        ) : (
            ...
            {filtered.map((app) => {
              ...
              return (
                <motion.div
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="p-5 rounded-xl bg-[#0f0f0f] border border-subtle hover:border-subtle[0.1] transition-colors cursor-pointer"
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-[14px] font-semibold text-foreground">{app.hotelName}</h3>
                    ...
                    <div className="flex items-center gap-4">
                      {app.creditScore !== null && (
                        <div className="text-right">
                          <p className="text-[11px] text-foreground-muted">AI Score</p>
                      ...
                      {app.recommendedLimit !== null && (
                        <div className="text-right">
                          <p className="text-[11px] text-foreground-muted">Recommended</p>
                          <p className="text-[14px] font-bold text-foreground">EGP {(app.recommendedLimit / 1_000_000).toFixed(1)}M</p>
                        </div>
                      )}
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <motion.div
            ...
            className="bg-[var(--background)] border border-subtle rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-y-auto"
          >
            <div className="p-6 border-b border-subtle flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-foreground-tertiary">{selected.crNumber} · {selected.governorate}</p>
                </div>
              </div>
              <button onClick={() => setSelected(null)} className="text-foreground-muted hover:text-foreground">
            ...
            <div className="p-6 space-y-6">
              {selected.creditScore !== null && (
                <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/10 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-[11px] text-foreground-tertiary">Recommended Limit</p>
                    <p className="text-[24px] font-bold text-foreground">EGP {(selected.recommendedLimit || 0).toLocaleString()}</p>
                  </div>
                </div>
              )}
              ...
              {selected.aiAnalysisReport && (
                <div>
                  <h3 className="text-[13px] font-semibold text-foreground/70 flex items-center gap-2 mb-3">
                    <BrainCircuit className="w-4 h-4" /> AI Risk Analysis
                  </h3>
                  <div className="p-4 rounded-xl bg-surface-raised border border-subtle text-[12px] text-foreground-tertiary leading-relaxed whitespace-pre-line">
                    {selected.aiAnalysisReport}
                  </div>
                </div>
              )}
              ...
              {selected.status === "FACTORING_REVIEW" && (
                <div className="flex items-center gap-3 pt-4 border-t border-subtle">
                  <button
                    ...
                    className="flex-1 py-3 bg-emerald-600 hover:bg-emerald-700 text-foreground text-[13px] font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                  >
