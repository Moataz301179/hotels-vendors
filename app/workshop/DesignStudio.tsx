...
export default function DesignStudio() {
  ...
  const injectClickHandler = useCallback(() => {
    ...
    style.textContent = '.ds-highlight { outline: 3px solid #7c3aed !important; outline-offset: 3px !important; cursor: pointer !important; }';
  ...
  const handleAiChat = async () => {
    ...
    try {
      ...
      if (data.suggestedCss && iframeRef.current?.contentDocument) {
        ...
        let style = doc.getElementById('design-studio-ai-css');
        if (!style) {
  ...
  // ── Render ─────────────────────────────────────────────────────
  return (
    <div className="h-screen flex flex-col bg-[#050508] text-white">
      
      {/* ═══ TOP BAR ═══ */}
      <header className="h-14 bg-[#0a0a12] border-b border-white/10 flex items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            {showDeviceDropdown && (
              <div className="absolute top-full mt-1 left-0 bg-[#0a0a12] rounded-lg border border-white/10 overflow-hidden z-50">
                {DEVICES.map(device => (
                  <button
                    key={device.name}
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`p-2 rounded-lg transition-colors ${isEditMode ? 'bg-[#7c3aed] text-white' : 'hover:bg-white/5 text-white/40'}`}
            title="Edit Preview"
          >
            <Edit3 size={18} />
          </button>
          ...
          <button 
            ...
            className={`p-2 rounded-lg transition-colors ${showColorPicker ? 'bg-[#7c3aed] text-white' : 'hover:bg-white/5 text-white/40'}`}
            title="Change Colors"
          >
            <Palette size={18} />
          </button>
          <div className="w-px h-6 bg-white/10 mx-1" />
          <button className="px-4 py-1.5 bg-[#7c3aed] text-white rounded-lg text-sm font-medium hover:bg-[#6d28d9] transition-colors">
            Publish
          </button>
        </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* ─── Left: Preview ─── */}
        <div className="flex-1 bg-[#050508] flex items-center justify-center p-8 relative">
          {/* Device Frame */}
          <div 
          ...
          {isEditMode && (
            <div className="absolute inset-0 pointer-events-none">
              <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-[#7c3aed] text-white px-4 py-2 rounded-full text-sm font-medium shadow-lg">
        ...
        {/* ─── Right: Sidebar ─── */}
        <aside className="w-96 bg-[#0a0a12] border-l border-white/10 flex flex-col">
            ...
            {[
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm transition-colors ${
                  activeTab === tab.id ? 'text-[#7c3aed] border-b-2 border-[#7c3aed]' : 'text-white/30 hover:text-white/60'
                }`}
              >
                <tab.icon size={14} />
                {tab.label}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {activeTab === 'comments' && (
              <div className="flex flex-col h-full">
                <div className="flex-1 overflow-y-auto p-4 space-y-3">
                  {comments.length === 0 ? (
                    ...
                    comments.map(comment => (
                      <div key={comment.id} className={`p-3 rounded-lg ${comment.author === 'ai' ? 'bg-[#7c3aed]/10 border border-[#7c3aed]/20' : 'bg-white/5'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-medium ${comment.author === 'ai' ? 'text-[#a78bfa]' : 'text-[#7c3aed]'}`}>
                ...
                {selectedElement && (
                  <div className="p-3 border-t border-white/10 bg-[#0a0a12]">
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm focus:outline-none focus:border-[#7c3aed] text-white placeholder-white/20"
                        onKeyPress={(e) => e.key === 'Enter' && handleAddComment()}
                      />
                      <button 
                        onClick={handleAddComment}
                        className="p-2 bg-[#7c3aed] text-white rounded-lg hover:bg-[#6d28d9]"
                      >
                        <Send size={14} />
                      </button>
                    </div>
            ...
            {activeTab === 'code' && (
              <div className="p-4">
                <div className="bg-[#050508] rounded-lg p-4 font-mono text-xs text-white/60 overflow-x-auto">
  ...
  --hv-muted: ${currentTheme.muted};
                ...
                {appliedCss && (
                  <div className="mt-3">
                    ...
                    <div className="bg-[#050508] rounded-lg p-4 font-mono text-xs text-white/60 overflow-x-auto max-h-40 overflow-y-auto">
                      <pre>{appliedCss}</pre>
                    </div>
                  </div>
                )}
      ...
      {/* ═══ BOTTOM: AI CHAT BAR ═══ */}
      <div className="h-auto bg-[#0a0a12] border-t border-white/10 p-4">
          ...
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-sm focus:outline-none focus:border-[#7c3aed] pr-12 text-white placeholder-white/20"
                onKeyPress={(e) => e.key === 'Enter' && handleAiChat()}
              />
              <Sparkles size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#7c3aed]" />
            </div>
            <button 
              onClick={handleAiChat}
              disabled={!chatInput.trim() || isAiTyping}
              className="px-6 py-3 bg-[#7c3aed] text-white rounded-xl text-sm font-medium hover:bg-[#6d28d9] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
      ...
      {selectedElement && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => { setSelectedElement(null); setEditText(''); }}>
          <div className="bg-[#0a0a12] rounded-xl border border-white/10 p-6 w-[28rem]" onClick={e => e.stopPropagation()}>
            ...
            <textarea
              ...
              className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-sm focus:outline-none focus:border-[#7c3aed] mb-4 text-white placeholder-white/20"
              rows={3}
            />
            <div className="flex gap-2">
              <button 
                onClick={applyTextEdit}
                className="flex-1 py-2 bg-[#7c3aed] text-white rounded-lg text-sm font-medium hover:bg-[#6d28d9]"
              >
                Apply
              </button>
              <button 
            ...
            <button 
              ...
              disabled={isAiTyping}
              className="w-full mt-2 py-2 bg-[#7c3aed]/10 text-[#a78bfa] rounded-lg text-sm hover:bg-[#7c3aed]/20 flex items-center justify-center gap-2 disabled:opacity-50"
            >
              <Sparkles size={14} />
              Ask AI to Improve
            </button>
      ...
      {showAssetUpload && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setShowAssetUpload(false)}>
          <div className="bg-[#0a0a12] rounded-xl border border-white/10 p-6 w-96" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
            ...
            <div 
              onClick={() => fileInputRef.current?.click()}
              className="border-2 border-dashed border-white/10 rounded-lg p-8 text-center cursor-pointer hover:border-[#7c3aed] transition-colors"
            >
