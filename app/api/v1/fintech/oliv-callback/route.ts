...
export const POST = apiRoute(async (request: NextRequest) => {
  // 1. Optional: Verify Oliv webhook auth token
  const authHeader = request.headers.get("authorization");
  const olivWebhookToken = process.env.OLIV_WEBHOOK_TOKEN;
  if (olivWebhookToken && authHeader !== `Bearer ${olivWebhookToken}`) {
  ...
  let newStatus: "DISBURSED" | "SETTLED" | "DEFAULTED" | "UNDER_REVIEW" = matchedRequest.status;
