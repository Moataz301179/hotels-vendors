...
export default function ProductDetailPage() {
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center gap-4">
        ...
        <Link
          href="/hotel/catalog"
          className="px-4 py-2 rounded-lg bg-[#8B0A1E] hover:bg-[#6B0512] text-white text-sm font-medium transition-colors"
        >
          Back to Catalog
        </Link>
      </div>
  ...
  const activeTier = bulkTiers.slice().reverse().find((t) => qty >= t.qty) || bulkTiers[0];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            <div className="relative aspect-square rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
              {product.images && product.images.length > 0 ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center bg-gradient-to-br from-[#8B0A1E]/5 to-transparent">
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                ...
                {product.supplierTier === "PREMIER" && (
                  <span className="px-3 py-1 rounded-full text-xs font-semibold bg-[#8B0A1E]/20 text-[#8B0A1E] border border-[#8B0A1E]/30">
                    Premier Supplier
                  </span>
                )}
              </div>
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span className="px-2.5 py-1 rounded-md bg-[#8B0A1E]/15 text-[#8B0A1E] text-xs font-semibold border border-[#8B0A1E]/25">
            ...
            <div className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.02]">
              <div className="flex items-baseline gap-3">
                ...
                <span className="text-sm text-white/40">for {qty} {product.unitOfMeasure}</span>
                {activeTier.discount > 0 && (
                  <span className="px-2 py-0.5 rounded-full bg-emerald-500/15 text-emerald-400 text-xs font-semibold border border-emerald-500/30">
                    Save {activeTier.discount}%
                  </span>
                )}
              </div>
              <p className="text-xs text-white/30 mt-1">
                {formatPrice(unitPrice)} / {product.unitOfMeasure} (bulk pricing applied)
              </p>
            </div>
                ...
                {bulkTiers.map((tier) => (
                  <button
                    ...
                    className={`p-2 rounded-lg border text-center transition-all ${
                      qty >= tier.qty
                        ? "bg-[#8B0A1E]/15 border-[#8B0A1E]/30 text-white"
                  ...
                  >
                    ...
                    {tier.discount > 0 && (
                      <div className="text-[9px] text-emerald-400">-{tier.discount}%</div>
                    )}
                  </button>
                ))}
              </div>
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center rounded-xl border border-white/[0.08] bg-white/[0.03] overflow-hidden">
                <button
                  ...
                  className="px-4 py-3 text-white/60 hover:text-white hover:bg-white/[0.05] transition-colors"
                >
                  <Minus className="w-4 h-4" />
                </button>
                <span className="px-4 py-3 text-sm font-medium text-white min-w-[4rem] text-center">
                  {qty}
                </span>
                <button
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-[#8B0A1E] hover:bg-[#6B0512] disabled:bg-white/[0.05] disabled:text-white/20 text-white font-medium transition-all active:scale-[0.98]"
              >
                {addedToCart ? (
                  <>
            ...
            <p className="text-xs text-white/30">
              ...
              {product.stockQuantity > 0 && (
                <span> · Available: <span className="text-white/50">{product.stockQuantity} {product.unitOfMeasure}</span></span>
              )}
            </p>

            {/* ETA Compliance Badge */}
            ...
            <div className="p-4 rounded-xl border border-white/[0.06] bg-white/[0.02]">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase ${
                  product.supplierTier === "PREMIER"
                    ? "bg-[#8B0A1E]/15 text-[#8B0A1E] border border-[#8B0A1E]/25"
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-[#8B0A1E]/15 border border-[#8B0A1E]/25 flex items-center justify-center text-sm font-bold text-[#8B0A1E]">
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
            <div className="grid grid-cols-2 md:grid-cols-4">
              {[
                ...
                <div
                  key={spec.label}
                  className={`p-4 ${i < 4 ? "border-b" : ""} ${i % 4 !== 3 ? "border-r" : ""} border-white/[0.04]`}
                >
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            ...
            <div key={item.title} className="flex items-start gap-3 p-4 rounded-xl border border-white/[0.06] bg-white/[0.02]">
              <item.icon className="w-5 h-5 text-[#8B0A1E] flex-shrink-0 mt-0.5" />
              <div>
