...
function SettingRow({ label, description, children }: { label: string; description: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center justify-between py-4 border-b border-border-invisible">
      <div>
...
function Toggle({ checked, onChange }: { checked: boolean; onChange: () => void }) {
  return (
    <button
      onClick={onChange}
      className={`w-10 h-5 rounded-full transition-colors relative ${checked ? "bg-accent-base" : "bg-surface-2"}`}
    >
      <span
    ...
    >
      ...
      <motion.div variants={fadeInUp} className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-foreground-muted mt-0.5">Manage platform configuration, preferences, and integrations</p>
        </div>
        ...
        <motion.div variants={fadeInUp} className="flex-1 min-w-0">
            ...
            <div className="rounded-xl border border-border-subtle bg-surface-1 p-5">
              ...
              <SettingRow label="Platform Name" description="Displayed across the application and emails">
                <input
                  ...
                  className="w-48 px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none focus:border-accent-base/50"
                />
              </SettingRow>
              <SettingRow label="Default Currency" description="Primary currency for all transactions">
                <select className="px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none">
                  <option className="bg-surface-2">EGP - Egyptian Pound</option>
                  <option className="bg-surface-2">USD - US Dollar</option>
                  <option className="bg-surface-2">EUR - Euro</option>
                </select>
              </SettingRow>
              <SettingRow label="Default Language" description="Interface language for new users">
                <select className="px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none">
                  <option className="bg-surface-2">English</option>
                  <option className="bg-surface-2">العربية</option>
                </select>
              </SettingRow>
              <SettingRow label="Time Zone" description="Default timezone for scheduling and reports">
                <select className="px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none">
                  <option className="bg-surface-2">Africa/Cairo (GMT+2)</option>
                  <option className="bg-surface-2">UTC</option>
                </select>
              </SettingRow>
            </div>
          )}
          ...
          {activeSection === "security" && (
            <div className="rounded-xl border border-border-subtle bg-surface-1 p-5">
              ...
              <SettingRow label="Session Timeout" description="Auto-logout after inactivity">
                <select className="px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none">
                  <option className="bg-surface-2">15 minutes</option>
                  <option className="bg-surface-2">30 minutes</option>
                  <option className="bg-surface-2">1 hour</option>
                  <option className="bg-surface-2">Never</option>
                </select>
              </SettingRow>
              <SettingRow label="Password Policy" description="Minimum requirements for user passwords">
                <select className="px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none">
                  <option className="bg-surface-2">Strong (12+ chars)</option>
                  <option className="bg-surface-2">Medium (8+ chars)</option>
                  <option className="bg-surface-2">Basic (6+ chars)</option>
                </select>
              </SettingRow>
              <SettingRow label="API Key" description="Your platform API key for integrations">
                <div className="flex items-center gap-2">
                  <code className="px-2 py-1 rounded bg-surface-2 text-[10px] text-foreground-muted font-mono">hv_live_••••••••••••</code>
              ...
              <div className="space-y-3">
                {([] as { name: string; email: string; role: string; status: string }[]).length === 0 ? (
                  <div className="py-6 text-center">
                    ...
                    <p className="text-xs text-foreground-muted">No team members yet.</p>
                    <p className="text-[10px] text-foreground-muted mt-1">Invite team members to collaborate.</p>
                  </div>
                ) : ([] as { name: string; email: string; role: string; status: string }[]).map((member, i) => (
                  <div key={i} className="flex items-center justify-between py-3 border-b border-border-invisible">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-surface-2 flex items-center justify-center text-[10px] text-foreground-muted font-medium">
            ...
            <div className="rounded-xl border border-border-subtle bg-surface-1 p-5">
              ...
              <SettingRow label="Address" description="Registered business address">
                <input
                  ...
                  className="w-64 px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none focus:border-accent-base/50"
                />
              </SettingRow>
          ...
          {activeSection === "billing" && (
            <div className="rounded-xl border border-border-subtle bg-surface-1 p-5">
              ...
              <div className="rounded-xl bg-surface-1 border border-border-invisible p-4 mb-4">
              ...
              <SettingRow label="Payment Method" description="Default payment for invoices">
                <span className="text-xs text-foreground-secondary">Visa ending in 4242</span>
              </SettingRow>
          ...
          {activeSection === "appearance" && (
            <div className="rounded-xl border border-border-subtle bg-surface-1 p-5">
              ...
              <SettingRow label="Theme" description="Interface color scheme">
                <select className="px-3 py-1.5 rounded-lg bg-surface-1 border border-border-subtle text-xs text-white focus:outline-none">
                  <option className="bg-surface-2">Dark</option>
                  <option className="bg-surface-2">Light</option>
                  <option className="bg-surface-2">System</option>
                </select>
              </SettingRow>
              <SettingRow label="Accent Color" description="Primary brand color">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded-full bg-accent-base border border-border-subtle" />
                  <span className="text-[11px] text-foreground-muted">var(--accent-base)</span>
                </div>
              </SettingRow>
