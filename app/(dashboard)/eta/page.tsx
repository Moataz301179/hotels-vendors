...
function StatusBadge({ status }: { status: string }) {
  const config: Record<string, { bg: string; text: string; dot: string; label: string }> = {
    ...
    VALIDATED: { bg: "bg-[#bef264]/10", text: "text-[#bef264]", dot: "bg-[#bef264]", label: "Validated" },
      ...
      <motion.div variants={fadeInUp} className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#bef264] hover:bg-[#bef264]/80 text-xs text-white font-medium transition-all"
          >
            <FileCheck size={14} />
            Submit Invoice
          </button>
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              action={
                <button
                  onClick={() => { setSubmitModalOpen(true); setSubmitStep("select"); setSubmitError(""); }}
                  className="px-4 py-2 rounded-lg bg-[#bef264] text-xs text-white font-medium"
                >
                  Submit Invoice
                </button>
              }
        ...
        <motion.div variants={fadeInUp} className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-5">
            ...
            <div className="flex items-center justify-center py-6">
              <div className="relative w-36 h-36">
                <svg className="w-full h-full -rotate-90" viewBox="0 0 100 100">
                  ...
                  <circle cx="50" cy="50" r="42" fill="none" stroke="#bef264" strokeWidth="8"
            ...
            <div className="space-y-2">
              ...
              <div className="h-1.5 rounded-full bg-white/[0.04] overflow-hidden"><div className="h-full rounded-full bg-[#bef264]" style={{ width: "85%" }} /></div>
            </div>
          </div>
        </motion.div>
      )}
              ...
              <AnimatePresence mode="wait">
                ...
                {submitStep === "submitting" && (
                  <motion.div key="submitting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12 flex flex-col items-center gap-3">
                    <Loader2 size={28} className="animate-spin text-[#bef264]" />
                ...
                {submitStep === "success" && submitResult && (
                  <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-6 space-y-4">
                    ...
                    <button onClick={closeSubmitModal} className="px-5 py-2 rounded-xl bg-[#bef264] hover:bg-[#bef264]/80 text-white text-sm font-medium transition-colors">Done</button>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
