    ...
    <main className="bg-slate-50 min-h-screen pt-16">
      <div className="max-w-3xl mx-auto px-5 lg:px-8 py-10">
        <header className="mb-8">
          <div className="text-[11px] font-semibold uppercase tracking-widest text-blue-600 flex items-center gap-1.5"><Calculator size={13} /> Dynamic Yield Calculator</div>
          <h1 className="text-3xl font-bold text-slate-900 mt-1">Early-payment discount simulator</h1>
        ...
        <div className="bg-white border border-slate-200 rounded-lg p-6 space-y-6">
          <div>
            <label className="block text-xs text-slate-600 mb-2 flex justify-between"><span>Invoice amount (EGP)</span><span className="font-semibold text-slate-900 tabular-nums">EGP {amount.toLocaleString()}</span></label>
          ...
          <div>
            <label className="block text-xs text-slate-600 mb-2 flex justify-between"><span>Factoring rate</span><span className="font-semibold text-slate-900">{rate.toFixed(2)}%</span></label>
          ...
          <div className="grid grid-cols-3 gap-3 pt-4 border-t border-slate-200">
            <div className="p-3 rounded bg-slate-50 border border-slate-200">
              ...
              <div className="text-sm font-bold text-slate-900 mt-1 tabular-nums">EGP {amount.toLocaleString()}</div>
            </div>
            <div className="p-3 rounded bg-slate-50 border border-slate-200">
              <div className="text-[10px] text-slate-500 uppercase">Fee ({rate}%)</div>
              <div className="text-sm font-bold text-slate-900 mt-1 tabular-nums">EGP {fee.toLocaleString()}</div>
            </div>
