...
export default function SupplierOnboardingPage() {
      ...
      <div className="min-h-screen bg-[#050505]">
        ...
        <div className="max-w-2xl mx-auto px-6 py-24 text-center">
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.5 }}>
            ...
            <div className="p-6 rounded-2xl bg-[#0a0a0a] border border-white/[0.08] text-left space-y-4 mb-10">
              <p className="text-[11px] font-semibold text-white/40 uppercase tracking-wider">What happens next?</p>
              <div className="flex items-start gap-4">
                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><ShieldCheck size={16} className="text-white" /></div>
                <div><p className="text-sm font-medium text-white">Document Verification</p><p className="text-sm text-white/40">Our compliance team reviews your commercial registration and tax documents.</p></div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><Factory size={16} className="text-white" /></div>
                <div><p className="text-sm font-medium text-white">Factory or Site Visit</p><p className="text-sm text-white/40">For PREMIER tier applicants, we conduct an on-site quality assessment.</p></div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center shrink-0"><Truck size={16} className="text-white" /></div>
                <div><p className="text-sm font-medium text-white">Onboarding Call</p><p className="text-sm text-white/40">We help you set up your product catalog, pricing, and delivery zones.</p></div>
              </div>
            </div>
            ...
            <div className="flex gap-4 justify-center">
              ...
              <button onClick={() => { setSubmitted(false); setStep(1); setForm({ name: "", legalName: "", taxId: "", commercialReg: "", description: "", address: "", city: "", governorate: "", phone: "", email: "", website: "", bankName: "", bankAccount: "", categories: [], minOrderValue: "", deliveryAreas: [], certifications: [] }); }} className="px-6 py-3 rounded-xl bg-[#8B0000] hover:bg-[#6B0000] text-white text-sm font-medium transition-colors">Apply Another Business</button>
            </div>
          </motion.div>
        </div>
        <MarketingFooter />
  ...
  return (
    <div className="min-h-screen bg-[#050505]">
      ...
      {/* Hero */}
      <section className="relative pt-32 pb-16 border-b border-white/[0.06]">
        <div className="max-w-6xl mx-auto px-6">
          <div className="max-w-2xl">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-[11px] font-semibold text-white/60 uppercase tracking-[0.15em] mb-6">
              <Store className="w-3 h-3" />
              Supplier Onboarding
            </div>
            <h1 className="text-[32px] md:text-[44px] font-bold text-white leading-[1.1] tracking-[-0.02em]">
              Join Egypt&apos;s Leading Hospitality Supply Network
            </h1>
            <p className="mt-5 text-[15px] text-white/50 leading-relaxed max-w-lg">
                    ...
                    <div key={s.id} className="flex items-center flex-1">
                      <div className="flex flex-col items-center">
                        <div className={`w-11 h-11 rounded-xl flex items-center justify-center transition-all ${
                          isActive ? "bg-[#8B0000] text-white" : isCompleted ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-white/5 text-white/30 border border-white/10"
                        }`}>
                          {isCompleted ? <CheckCircle2 size={20} /> : <Icon size={20} />}
                        </div>
                        <span className={`text-[11px] mt-2 font-medium ${isActive ? "text-white" : isCompleted ? "text-emerald-400/60" : "text-white/30"}`}>{s.label}</span>
                      </div>
                      {i < STEPS.length - 1 && (
                        <div className={`flex-1 h-[2px] mx-3 rounded-full ${isCompleted ? "bg-emerald-500/30" : "bg-white/[0.05]"}`} />
                      )}
                    </div>
                  );
                })}
            ...
            <div className="rounded-2xl border border-white/[0.08] bg-[#0a0a0a] overflow-hidden">
                ...
                >
                  {step === 1 && (
                    <div className="space-y-6">
                      <h3 className="text-[15px] font-semibold text-white mb-6 flex items-center gap-2.5">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div className="sm:col-span-2">
                          ...
                          <input
                            ...
                            placeholder="e.g. Nile Fresh Foods"
                            className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-[14px] placeholder:text-white/20 focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/10 transition-colors"
                          />
                        </div>
                        <div className="sm:col-span-2">
                          <label className="block text-[13px] font-medium text-white/70 mb-2">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <textarea
                            ...
                            placeholder="Briefly describe your company, products, and target market..."
                            rows={3}
                            className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-[14px] placeholder:text-white/20 focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/10 transition-colors resize-none"
                          />
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div>
                          ...
                          <select
                            ...
                            className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-[14px] focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/10 transition-colors appearance-none"
                          >
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full pl-10 pr-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-[14px] placeholder:text-white/20 focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/10 transition-colors"
                            />
                          </div>
                        </div>
                        <div>
                  ...
                  {step === 3 && (
                    <div className="space-y-6">
                      ...
                      <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 mb-4">
                        <p className="text-[13px] text-amber-400/70 flex items-start gap-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <input
                            ...
                            placeholder="EGXXXXXXXXXXXXXXXXXXXXXXXX"
                            className="w-full px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white text-[14px] placeholder:text-white/20 focus:border-white/20 focus:outline-none focus:ring-1 focus:ring-white/10 transition-colors"
                          />
                        </div>
                      </div>
                            ...
                            return (
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all ${
                                  selected ? "bg-white/10 text-white border border-white/20" : "bg-white/[0.03] text-white/40 border border-white/[0.06] hover:border-white/[0.12] hover:text-white/60"
                                }`}
                              >
                                {cat}
                              </button>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {DELIVERY_AREAS.map((area) => {
                            ...
                            return (
                              <button
                                key={area}
                                onClick={() => toggleArray("deliveryAreas", area)}
                                className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all ${
                                  selected ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-white/[0.03] text-white/40 border border-white/[0.06] hover:border-white/[0.12] hover:text-white/60"
                                }`}
                              >
                                {area}
                              </button>
                          ...
                          {CERTIFICATIONS.map((cert) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-all ${
                                  selected ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-white/[0.03] text-white/40 border border-white/[0.06] hover:border-white/[0.12] hover:text-white/60"
                                }`}
                              >
                                {cert}
                              </button>
                      ...
                      <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.08]">
                        <label className="flex items-start gap-3 cursor-pointer">
                          ...
                          <span className="text-[13px] text-white/50">
                            ...
                            and{" "}
                            <span className="text-white underline cursor-pointer">Supplier Agreement</span>.
                          </span>
                        </label>
                      </div>
                    </div>
              ...
              <div className="flex items-center justify-between px-8 py-5 border-t border-white/[0.06]">
                <button
                  onClick={prevStep}
                  disabled={step === 1}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-white/40 hover:text-white hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-[13px]"
                >
                ...
                {step < 5 ? (
                  <button
                    onClick={nextStep}
                    className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#8B0000] hover:bg-[#6B0000] text-white text-[13px] font-medium transition-colors"
                  >
                  ...
                  <button
                    ...
                    disabled={submitting}
                    className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-[#8B0000] hover:bg-[#6B0000] disabled:opacity-50 text-white text-[13px] font-medium transition-colors"
                  >
  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center justify-between px-5 py-3.5 rounded-xl bg-[#0a0a0a] border border-white/[0.08] text-white text-[13px] font-medium"
      >
...
function ReviewSection({ title, icon: Icon, children }: { title: string; icon: React.ElementType; children: React.ReactNode }) {
  return (
    <div className="p-5 rounded-xl bg-white/[0.03] border border-white/[0.08]">
      <h4 className="text-[12px] font-semibold text-white/40 uppercase tracking-wider mb-4 flex items-center gap-2">
