      ...
      <div className="min-h-screen bg-black">
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center space-y-6 max-w-md">
          ...
          <div>
            ...
            <p className="text-gray-400 mt-2">Thank you for applying to join Hotels Vendors. Our team will review your application within 2-3 business days.</p>
          </div>
    ...
    <div className="min-h-screen bg-black">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="text-center mb-8">
              ...
              <p className="text-sm text-gray-400 mt-1">Join Egypt&apos;s leading hospitality procurement platform</p>
            </div>
          ...
          return (
            <div key={s.id} className="flex items-center flex-1">
              <div className="flex flex-col items-center">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isActive ? "bg-[#8B0000] text-white ring-2 ring-[#8B0000]/30" : isCompleted ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/20" : "bg-white/5 text-gray-500 border border-white/10"}`}>
                  {isCompleted ? <CheckCircle2 size={18} /> : <Icon size={18} />}
                </div>
                <span className={`text-[10px] mt-1.5 font-medium ${isActive ? "text-white" : isCompleted ? "text-emerald-400/60" : "text-gray-500"}`}>{s.label}</span>
              </div>
              {i < STEPS.length - 1 && <div className={`flex-1 h-[2px] mx-2 rounded-full ${isCompleted ? "bg-emerald-500/30" : "bg-white/5"}`} />}
            </div>
          );
        })}
      </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-[#111] overflow-hidden">
          ...
          <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <input type="text" value={form.name} onChange={(e) => updateField("name", e.target.value)} placeholder="e.g. Nile Fresh Foods" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Legal Name</label>
                    <input type="text" value={form.legalName} onChange={(e) => updateField("legalName", e.target.value)} placeholder="Registered legal entity name" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Tax ID <span className="text-red-400">*</span></label>
                    <input type="text" value={form.taxId} onChange={(e) => updateField("taxId", e.target.value.replace(/\D/g, ""))} placeholder="9-digit Egyptian tax ID" maxLength={9} className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Commercial Reg. No.</label>
                    <input type="text" value={form.commercialReg} onChange={(e) => updateField("commercialReg", e.target.value)} placeholder="Commercial registration number" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Company Description</label>
                    <textarea value={form.description} onChange={(e) => updateField("description", e.target.value)} placeholder="Briefly describe your company, products, and target market..." rows={3} className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm resize-none" />
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    ...
                    <input type="text" value={form.address} onChange={(e) => updateField("address", e.target.value)} placeholder="Full street address" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">City <span className="text-red-400">*</span></label>
                    <input type="text" value={form.city} onChange={(e) => updateField("city", e.target.value)} placeholder="e.g. 6th of October" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Governorate <span className="text-red-400">*</span></label>
                    <select value={form.governorate} onChange={(e) => updateField("governorate", e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm appearance-none">
                      <option value="" className="bg-[#111]">Select governorate</option>
                      {GOVERNORATES.map((g) => (<option key={g} value={g} className="bg-[#111]">{g}</option>))}
                    </select>
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Phone <span className="text-red-400">*</span></label>
                    <div className="relative">
                      <Phone size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
                      <input type="tel" value={form.phone} onChange={(e) => updateField("phone", e.target.value)} placeholder="+20 1XX XXX XXXX" className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                    </div>
                  </div>
                  <div>
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Email <span className="text-red-400">*</span></label>
                    <div className="relative">
                      <Mail size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
                      <input type="email" value={form.email} onChange={(e) => updateField("email", e.target.value)} placeholder="contact@company.com" className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                    </div>
                  </div>
                  <div className="sm:col-span-2">
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Website</label>
                    <div className="relative">
                      <Globe size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600" />
                      <input type="url" value={form.website} onChange={(e) => updateField("website", e.target.value)} placeholder="https://www.company.com" className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Bank Name <span className="text-red-400">*</span></label>
                    <input type="text" value={form.bankName} onChange={(e) => updateField("bankName", e.target.value)} placeholder="e.g. National Bank of Egypt" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                  <div className="sm:col-span-2">
                    <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">IBAN / Account Number <span className="text-red-400">*</span></label>
                    <input type="text" value={form.bankAccount} onChange={(e) => updateField("bankAccount", e.target.value)} placeholder="EGXXXXXXXXXXXXXXXXXXXXXXXX" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                  </div>
                </div>
              </div>
            )}
            ...
            {step === 4 && (
              <div className="space-y-5">
                ...
                <div>
                  <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-2 block">Delivery Areas</label>
                  <div className="flex flex-wrap gap-2">
                    {DELIVERY_AREAS.map((area) => {
                      const selected = form.deliveryAreas.includes(area);
                      return (<button key={area} onClick={() => toggleArray("deliveryAreas", area)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selected ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-white/5 text-gray-400 border border-white/10 hover:border-white/15"}`}>{area}</button>);
                    })}
                  </div>
                </div>
                <div>
                  <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-2 block">Certifications</label>
                  <div className="flex flex-wrap gap-2">
                    {CERTIFICATIONS.map((cert) => {
                      const selected = form.certifications.includes(cert);
                      return (<button key={cert} onClick={() => toggleArray("certifications", cert)} className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selected ? "bg-amber-500/10 text-amber-400 border border-amber-500/20" : "bg-white/5 text-gray-400 border border-white/10 hover:border-white/15"}`}>{cert}</button>);
                    })}
                  </div>
                </div>
                <div>
                  <label className="text-[11px] text-gray-400 uppercase tracking-wider mb-1.5 block">Minimum Order Value (EGP)</label>
                  <input type="number" value={form.minOrderValue} onChange={(e) => updateField("minOrderValue", e.target.value)} placeholder="e.g. 5000" className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-600 focus:border-[#8B0000]/40 focus:outline-none transition-colors text-sm" />
                </div>
              </div>
            )}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/10">
          <button onClick={prevStep} disabled={step === 1} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:text-white hover:bg-white/5 disabled:opacity-30 disabled:cursor-not-allowed transition-colors text-sm">
...
function ReviewRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between text-sm">
      ...
      <span className="text-gray-400 text-right max-w-[60%]">{value}</span>
    </div>
  );
}
