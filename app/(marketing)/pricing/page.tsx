    ...
    <main style={{ backgroundColor: "var(--background)", color: "var(--text-primary)", minHeight: "100vh", fontFamily: "var(--font-sans)" }}>
      ...
      <section className="pt-28 pb-16 relative overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 text-center">
          <span className="text-[11px] font-medium text-primary/30 uppercase tracking-[0.15em] mb-3 block">Layer 2 — Financing & Monetization</span>
          ...
          <p className="text-[15px] text-primary/40 max-w-2xl mx-auto leading-relaxed">
            HotelsVendors charges a <strong className="text-primary/60">monthly subscription</strong> based on your property size and room count — <strong className="text-primary/60">not per transaction</strong>. Platform revenue is generated through <strong className="text-primary/60">factoring funding fees (HV)</strong>, fully disclosed under FRA and ETA compliance frameworks.
          </p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-6">
          <div className="rounded-2xl p-6 max-w-3xl mx-auto" style={{ backgroundColor: surface, border: `1px solid ${accentBorder}` }}>
            ...
            <div className="space-y-4 text-[12px] leading-relaxed" style={{ color: "rgba(255,255,255,0.5)" }}>
              <p>
                <strong className="text-primary/70">1. Subscription Fees:</strong> Hotels pay a monthly subscription based on property size, room count, and number of properties in your chain. No per-transaction fees. No hidden charges.
              </p>
              <p>
                <strong className="text-primary/70">2. Factoring Funding Fees (HV):</strong> Revenue is generated through the factoring process. When a supplier opts for early payment via reverse factoring, a <strong className="text-primary/70">Funding Fee (HV)</strong> of 0.8%–1.8% is embedded in the factoring spread based on your tier. This is fully compliant with FRA anti-fraud regulations and ETA e-invoicing disclosure requirements.
              </p>
              <p>
                <strong className="text-primary/70">3. No Hidden Costs:</strong> Your subscription covers the full platform — from AI forecasting to ETA compliance to settlement. Revenue is generated when value is created through the factoring process, not on every purchase order.
              </p>
              <div className="mt-3 p-3 rounded-lg" style={{ backgroundColor: accentMuted, border: `1px solid ${accentBorder}` }}>
                <p className="text-[11px]" style={{ color: "var(--text-muted)" }}>
                  <strong className="text-primary/60">Regulatory Note:</strong> Funding Fees (HV) are disclosed in accordance with FRA Anti-Fraud Compliance (three-way matching gate: PO + ETA UUID + Signed Digital GRN) and ETA e-invoicing regulations. All fees are itemized in the factoring bid terms and settlement documentation.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="py-16">
        <div className="mx-auto max-w-7xl px-6">
          <p className="text-[10px] font-medium text-primary/20 uppercase tracking-[0.15em] mb-6 text-center">Subscription Plans — Based on Your Property Size</p>
          <div className="grid md:grid-cols-3 gap-4">
            {tiers.map((tier) => (
              ...
              >
                ...
                <h3 className="text-[18px] font-medium text-primary mb-1">{tier.name}</h3>
                <div className="flex items-baseline gap-1 mb-2">
                  <span className="text-[32px] font-semibold text-primary">{tier.price}</span>
                  {tier.period && <span className="text-[12px] text-primary/30">{tier.period}</span>}
                </div>
                <p className="text-[12px] text-primary/35 mb-4">{tier.desc}</p>
                ...
                <div className="mb-4 p-3 rounded-lg space-y-2" style={{ backgroundColor: "var(--bg-surface-2)", border: `1px solid ${borderSubtle}` }}>
                  {tier.metrics.map((m) => (
                    <div key={m.label} className="flex items-center justify-between">
                      <span className="text-[10px] text-primary/35">{m.label}</span>
                      <span className="text-[11px] font-semibold text-primary/70">{m.value}</span>
                    </div>
                  ))}
                </div>
                ...
                <ul className="space-y-2.5 mb-6 flex-1">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5">
                      ...
                      <span className="text-[11px] text-primary/50">{f}</span>
                    </li>
                  ))}
                </ul>
                <Link
                  ...
                  style={tier.highlighted
                    ? { backgroundColor: accent, color: "var(--text-primary)" }
      ...
      <section className="py-16" style={{ backgroundColor: "var(--background)" }}>
        <div className="mx-auto max-w-7xl px-6 text-center">
          <h2 className="text-[11px] font-medium text-primary/30 uppercase tracking-[0.15em] mb-6">Frequently Asked</h2>
          <div className="grid md:grid-cols-3 gap-6 max-w-3xl mx-auto text-left">
            {[
              ...
              <div key={faq.q} className="rounded-xl p-5" style={{ backgroundColor: surface, border: `1px solid ${borderSubtle}` }}>
                <h3 className="text-[13px] font-medium text-primary mb-2">{faq.q}</h3>
                <p className="text-[12px] text-primary/35 leading-relaxed">{faq.a}</p>
              </div>
            ))}
          </div>
        </div>
