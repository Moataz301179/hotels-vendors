...
const STEPS: Step[] = [
  {
    ...
    supplierView: "Download INVO → Enter phone → KYC auto-fills → Ready in 60s",
    terminalOut: [
  ...
  },
  {
    ...
    desc: "Approved purchase orders instantly pushed to the supplier's INVO Mobile App. Warehouse staff scan, pack, and issue e-Waybills with zero manual data entry.",
    ...
    supplierView: "Push alert on INVO → Accept with one tap → Scan items → Pack → Ship",
    terminalOut: [
      "[Orders] PO #HV-8812 created for Luxe Linen Co. — EGP 14,400",
      "[Auth] Authority matrix: Manager approved · Tier 2 OK",
      "[Mobile] Push notification sent to INVO: 'New Order #HV-8812'",
  ...
  },
  {
    ...
    desc: "Suppliers request reverse factoring via INVO Mobile. AI compliance agents audit the invoice and trigger disbursement in 38–48 hours at 1.5–3% rate, while hotels preserve standard 60–90 day payment cycles.",
        ...
        <div className="inline-flex rounded-xl border border-white/[0.08] bg-surface-1 p-1">
          {(["hotel", "supplier"] as ViewMode[]).map((v) => (
            ...
            >
              ...
              {v === "hotel" ? "Hotel View (Web)" : "Supplier View (INVO Mobile)"}
            </button>
          ))}
        </div>
      </div>
