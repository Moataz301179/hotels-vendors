...
export function btnCls(variant: BtnVariant = "primary", size: "sm" | "md" | "lg" = "md") {
  ...
  const variants: Record<BtnVariant, string> = {
    ...
    accent:
      "bg-brass-500 text-ink-950 font-semibold hover:bg-brass-400 shadow-[0_0_16px_var(--accent-glow-secondary)] dark:bg-brass-500 dark:text-white dark:hover:bg-brass-600 dark:shadow-[0_0_20px_var(--accent-glow-primary)]",
    outline:
...
export function ComplianceChip({ ok, label, Icon }: { ok?: boolean; label: string; Icon?: React.ComponentType<{ size?: number }> }) {
  return (
    <div className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-medium" style={{
      backgroundColor: ok !== false ? "rgba(34,197,94,0.12)" : "rgba(239,68,68,0.12)",
      color: ok !== false ? "#22c55e" : "#ef4444",
      border: "1px solid",
      borderColor: ok !== false ? "rgba(34,197,94,0.25)" : "rgba(239,68,68,0.25)",
    }}>
      {Icon && <Icon size={12} />}
      {label}
    </div>
