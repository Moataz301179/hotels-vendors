...
export default function OrderDetail({ orderId, mode }: { orderId: string; mode: "hotel" | "supplier" }) {
  ...
  const { data, user, decideOrder, advanceFulfillment } = useApp();
  const toast = (msg: string, _type?: string) => console.log("[toast]", msg);
  ...
  const invoice = useMemo(
    () => (order && (data && data.invoices) ? data.invoices.find((i) => i.orderId === order.id) : undefined),
    [data, order]
  );
  const financing = useMemo(
    () => (order && (data && data.financing) ? data.financing.find((f) => f.orderId === order.id) : undefined),
    [data, order]
  );
  const delivery = useMemo(
    () => (order && (data && data.deliveries) ? data.deliveries.find((d) => d.orderId === order.id) : undefined),
    [data, order]
  );
  ...
  const doApprove = () => {
    decideOrder?.(order.id, "approve", note);
    toast(t("orders.detail.approveDone"));
  ...
  const doReject = () => {
    ...
    decideOrder?.(order.id, "reject", note.trim());
    toast(t("orders.detail.rejectDone"));
    setRejectOpen(false);
    setNote("");
    setNoteErr("");
  };
  const doShip = () => {
    advanceFulfillment?.(order.id, "shipped");
    toast(t("orders.detail.shipDone"));
    setShipOpen(false);
  };
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-[13px] text-ink-500 dark:text-ink-400">
                {t("cart.approver")}:{" "}
                <strong>{(Array.isArray(order.approval.required) ? order.approval.required.map((r) => t(`role.${r}`)).join(" + ") : "")}</strong>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <Btn
              onClick={() => {
                ...
                toast(t("orders.detail." + (nextFulfillMsg[order.fulfillment] ?? "ackDone")));
              }}
            >
                ...
                <KV
                  ...
                  v={
                    <span className="tnum">
                      ...
                      <span className="ms-2 text-xs font-normal text-ink-400">{relDay(delivery.eta)}</span>
                    </span>
                  }
                />
                ...
                <div className="mt-4 space-y-2">
                  {(delivery?.exceptions ?? []).map((ex, i) => (
                    <div key={i} className="rounded border border-red-600/25 bg-red-600/5 px-3 py-2 text-[13px] text-red-800 dark:text-red-300">
                      <span className="tnum me-2 text-xs opacity-70">{fmtDateTime(ex.at)}</span>
                      {ex.note}
                    </div>
                  ))}
                </div>
