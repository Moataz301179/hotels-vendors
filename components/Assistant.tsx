...
export default function Assistant() {
  ...
  const answer = useMemo(() => {
    return (raw: string): { text: string; items?: Item[] } => {
      if (!data) return { text: t("assistant.nothing") };
      ...
      if (/approv|pending|authority|escalat|اعتماد|صلاح/.test(q)) {
        ...
        return {
          ...
          items: pending.map((o) => ({
            label: o.po,
            meta: `${supplierById(o.supplierId)?.name} · ${fmtMoney(o.total, lang)} · ${(Array.isArray(o.approval.required) ? o.approval.required.map((r) => t(`role.${r}`)).join(" + ") : "")}`,
      ...
      if (/transit|deliver|eta|shipping|shipment|tracking|توصيل|طريق|شحن|لوجست/.test(q)) {
        ...
        return {
          ...
          items: live.map((d) => ({
            label: d.id,
            meta: `${t(`state.${d.status}`)} · ETA ${fmtDateTime(d.eta)}${d.delayed ? ` · ${t("state.delayed")}` : ""}`,
            href: "/eta-compliance",
          })),
        };
      }
      ...
      if (/invoice|bill|payment due|فاتورة/.test(q)) {
        ...
        return {
          ...
          items: inv.map((i) => ({
            label: i.number,
            meta: `${t(`state.${i.status}`)} · ${fmtMoney(i.total ?? i.amount, lang)} · ${t("invoices.col.due")} ${new Intl.DateTimeFormat(lang === "ar" ? "ar-EG" : "en-GB", { day: "2-digit", month: "short" }).format(new Date(i.dueDate))}`,
            href: "/invoices",
          })),
        };
      }
      ...
      /* educational marketing: distinguished role */
      if (/distinguish|infrastructure|role|why hotelsvendors|neutral|broker|wholesal|ممي|دور|محايد|وسيط/.test(q)) {
        return {
          text: t("assistant.ansDistinguished"),
          items: [
            { label: lang === "ar" ? "الإحاطة التعليمية الكاملة" : "Educational Showcase", href: "/#educational-showcase" },
            { label: t("nav.marketplace"), href: "/marketplace" },
          ],
        };
      }

      /* educational marketing: why suppliers use it */
      if (/why supplier|suppliers benefit|supplier use|for supplier|لماذا المورد|مكاسب المورد|فائدة المورد/.test(q)) {
        return {
          text: t("assistant.ansSuppliers"),
          items: [
            { label: t("nav.central"), href: "/supplier-central" },
            { label: t("nav.financing"), href: "/financing" },
          ],
        };
      }

      /* educational marketing: why hotels benefit */
      if (/why hotel|hotels benefit|hotel benefit|for hotel|لماذا الفندق|مكاسب الفندق|فائدة الفندق/.test(q)) {
        return {
          text: t("assistant.ansHotels"),
          items: [
            { label: t("nav.marketplace"), href: "/marketplace" },
            { label: t("nav.orders"), href: "/orders" },
            { label: t("nav.eta"), href: "/eta-compliance" },
          ],
        };
      }
  ...
  return (
    <>
      ...
      <button
        ...
        aria-expanded={open}
        className="fixed bottom-5 end-5 z-[55] flex h-14 w-14 items-center justify-center rounded-full bg-ink-950 text-brass-300 shadow-[0_0_24px_rgba(6,182,212,0.40)] ring-2 ring-brass-400/60 transition-all hover:scale-110 dark:bg-black dark:text-brass-300 dark:ring-brass-400/70 dark:shadow-[0_0_28px_rgba(192,132,252,0.55)]"
      >
        {open ? <IcX className="text-2xl" /> : <IcSpark className="text-2xl" />}
        {!open && cartCount > 0 ? (
          <span className="tnum absolute -top-1 -end-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-brass-500 px-1 text-[10px] font-bold text-ink-950 dark:text-white shadow-[0_0_10px_var(--accent-glow-primary)]">
            {cartCount}
          </span>
        ) : null}
      </button>
      ...
      {open ? (
        ...
        >
          ...
          >
            ...
            <button
              type="submit"
              className="flex h-10 shrink-0 items-center gap-1.5 rounded bg-brass-500 px-3.5 text-[13px] font-semibold text-ink-950 transition-colors hover:bg-brass-400 dark:bg-brass-500 dark:text-white dark:hover:bg-brass-600"
            >
              {t("assistant.send")}
