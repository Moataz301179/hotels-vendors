...
export function UserDropdown({ user }: { user?: UserData | null }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[#8B0000] to-[#5C0000] flex items-center justify-center text-white text-xs font-bold ring-2 ring-white/10 flex-shrink-0">
          {initials}
        </div>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-56 bg-[#1a1a1a] border border-white/[0.08] rounded-xl shadow-2xl overflow-hidden animate-fade-in-up z-50">
          <div className="p-3 border-b border-white/[0.06]">
            ...
            {user?.tenantName && (
              <p className="text-[10px] text-[#C9A227] mt-0.5 truncate">{user.tenantName}</p>
            )}
          </div>
          <div className="p-1.5">
            <Link
