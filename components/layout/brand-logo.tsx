"use client";

import { cn } from "@/lib/utils";

interface BrandLogoProps {
  className?: string;
  variant?: "dark" | "light";
  size?: "sm" | "md" | "lg" | "xl";
}

const SIZE_MAP = {
  sm: "w-5 h-6",
  md: "w-7 h-8",
  lg: "w-9 h-10",
  xl: "w-14 h-16",
};

const STROKE_MAP = {
  dark: "#ffffff",
  light: "#1a1a1a",
};

/**
 * Adaptive brand logo — outline style horse head.
 * Use variant="dark" on dark backgrounds (white stroke).
 * Use variant="light" on light backgrounds (black stroke).
 */
export function BrandLogo({
  className,
  variant = "light",
  size = "md",
}: BrandLogoProps) {
  const stroke = STROKE_MAP[variant];
  const sizeClass = SIZE_MAP[size];

  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 120 140"
      className={cn(sizeClass, className)}
      fill="none"
      aria-label="Hotels Vendors"
    >
      {/* Main horse head silhouette outline */}
      <path
        d="M85 15 C95 15, 105 22, 110 32 L108 42 C112 48, 115 55, 112 62 L105 75 C100 82, 92 85, 84 83 L72 88 C65 92, 58 98, 55 105 L48 120 C45 128, 38 135, 30 138 L20 135 C15 132, 12 125, 15 118 L22 98 C25 90, 32 85, 40 82 L52 78 C58 76, 62 72, 60 66 L55 52 C52 42, 58 32, 68 28 L78 22 C80 18, 82 15, 85 15Z"
        stroke={stroke}
        strokeWidth="3"
        strokeLinejoin="round"
        opacity="0.95"
      />
      {/* Inner detail lines */}
      <path
        d="M58 32 C62 28, 68 26, 74 28 L80 42 C82 48, 80 55, 75 60 L65 70 C60 75, 55 78, 50 76 L42 72 C38 70, 36 65, 38 60 L45 45 C48 38, 52 34, 58 32Z"
        stroke={stroke}
        strokeWidth="2"
        strokeLinejoin="round"
        opacity="0.45"
      />
      {/* Eye */}
      <circle cx="88" cy="38" r="3.5" fill={stroke} opacity="0.8" />
      {/* Jaw line detail */}
      <path
        d="M25 125 C30 122, 35 120, 40 122"
        stroke={stroke}
        strokeWidth="2"
        strokeLinecap="round"
        opacity="0.5"
      />
    </svg>
  );
}
