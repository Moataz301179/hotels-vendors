...
const ROLE_CONFIG: Record<string, { label: string; badgeColor: string }> = {
  admin: { label: "Platform Admin", badgeColor: "bg-[#bef264]" },
    ...
    <header className="h-14 sm:h-16 flex items-center justify-between px-3 sm:px-6 sticky top-0 z-30 bg-[#0a0a0a]/80 backdrop-blur-xl border-b border-white/[0.06]">
      ...
      <div className="flex-1 max-w-xl mx-2 sm:mx-4 lg:mx-8">
        <div className="relative group flex items-center">
          <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-[#bef264] transition-colors z-10" />
          <input
            type="text"
            placeholder="Search orders, suppliers, products..."
            className="w-full h-9 pl-9 pr-20 sm:pr-24 rounded-lg text-sm text-white placeholder:text-white/20 bg-white/[0.04] border border-white/[0.08] outline-none focus:border-[#bef264]/40 focus:ring-1 focus:ring-[#bef264]/10 transition-all"
          />
      ...
      <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
        ...
        >
          ...
          {totalItems > 0 && (
            <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 rounded-full bg-[#bef264] text-[10px] font-bold text-white flex items-center justify-center ring-2 ring-[#121212]">
