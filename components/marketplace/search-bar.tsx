      ...
      <form onSubmit={handleSubmit} className="relative">
        <div
          className={`flex items-center gap-3 rounded-xl border bg-white/[0.03] px-4 py-3 transition-all duration-200 ${
            isFocused
              ? "border-[#800000]/50 shadow-[0_0_20px_rgba(128,0,0,0.15)]"
        ...
        >
          ...
          <button
            ...
            className={`relative p-1.5 rounded-lg transition-colors ${
              showFilters ? "bg-[#800000]/20 text-[#ff4d4d]" : "text-white/30 hover:text-white/60"
            }`}
          >
            <SlidersHorizontal className="w-4 h-4" />
            {activeFilterCount > 0 && (
              <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#800000] text-white text-[9px] font-bold flex items-center justify-center">
                {activeFilterCount}
              </span>
            )}
          </button>
          <button
            type="submit"
            className="px-4 py-1.5 rounded-lg bg-[#800000] hover:bg-[#990000] text-white text-sm font-medium transition-colors"
          >
            Search
          </button>
        </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {trending.map((s) => (
                      <button
                        key={s}
                        onClick={() => handleSuggestionClick(s)}
                        className="px-2.5 py-1 rounded-lg bg-[#800000]/10 border border-[#800000]/20 text-xs text-[#ff4d4d]/80 hover:text-[#ff4d4d] hover:border-[#800000]/40 transition-colors"
                      >
                        {s}
                      </button>
                    ))}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className="px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-sm text-white placeholder:text-white/20 outline-none focus:border-[#800000]/50 transition-colors"
      />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <select
        ...
        className="px-3 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-sm text-white outline-none focus:border-[#800000]/50 transition-colors appearance-none"
        style={{
